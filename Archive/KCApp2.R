...
server <- function(input, output) {
  ...
  modresult4<- reactive({
    pred = predict.lm(mod, newdata = data.frame(Sex=rep(input$sex,11), Age_years = rep(input$age,11), WaistCMAvg = rep(input$waist,11) ,Race = rep(input$race,11), Tanita.Avg_percentbodyfat = c((input$tanita-5):(input$tanita+5))), interval = "prediction")
    guesses = data.frame(
      # Sex = input$sex,
      # Age_years = input$age,
      # Race = input$race,
    ...
    modresult5<- reactive({
    pred = predict.lm(mod, newdata = data.frame(Sex=rep(input$sex,11), Age_years = rep(input$age,11), WaistCMAvg = rep(input$waist,11) ,Race = rep(input$race,11), Tanita.Avg_percentbodyfat = rep(input$tanita,11)), interval = "prediction")
    guesses = data.frame(
      # Sex = input$sex,
      # Age_years = input$age,
      # Race = input$race,
    ...
    modresult6 <- reactive({
      pred = predict.lm(mod, newdata = data.frame(Sex=rep(input$sex,2), Age_years = rep(input$age,2), WaistCMAvg = rep(input$waist,2),Race = c("White","Other"), Tanita.Avg_percentbodyfat = rep(input$tanita,2)), interval = "prediction")
      guesses = data.frame(
        # Sex = input$sex,
        # Age_years = input$age,
        # Race = input$race,
